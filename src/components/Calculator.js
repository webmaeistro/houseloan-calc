...
export const Calculator = () => {
	...
	const handleHouseloanDataChange = (depositAmount, purchasingHousePrice, houseloanTerm, interestRate) => {

		//Set initial values for the whole houseloan term
		const amountToBorrow = purchasingHousePrice - depositAmount;
		const monthlyPayment = ((interestRate / 100 / 12)*amountToBorrow) / (1 - (Math.pow(( 1+ (interestRate / 100 / 12)),((0 - houseloanTerm) * 12))));
		const totalRepaid = monthlyPayment * 12 * houseloanTerm;
		...
		//Loop each year of the houseloan term
		for(let i = 1; i <= houseloanTerm; i++) {
			...
			//There's always around kr10 left at the end which forces the fraph to go into minus. This just rounds the last figure off at kr0.00.
			if(i === houseloanTerm) {
		...
		setHouseloanTerm(houseloanTerm);
	...
	useEffect(() => {
		handleHouseloanDataChange(depositAmount, purchasingHousePrice, houseloanTerm, interestRate);
	},[depositAmount, purchasingHousePrice, houseloanTerm, interestRate]);
	...
	return(
		<div>
			...
			/>
			<CalculatorControls 
				handleHouseloanDataChange={handleHouseloanDataChange} 
				depositAmount={depositAmount}
				purchasingHousePrice={purchasingHousePrice}
				houseloanTerm={houseloanTerm}
