...
const graphOption = {
    ...
    tooltips: {
    	callbacks: {
        	label: function(tooltipItems, data, _index) {
            	return parseFloat(tooltipItems.value).toLocaleString('nb-NO', {style:'currency', currency:'NOK'}) + ' ' + data.datasets[tooltipItems.datasetIndex].label;
        	}
    	}
    }
}
...
export const PaymentGraph = props => {
	...
	barGraphData.datasets[1].data = [];
  	for(let i = 0; i <= props.houseloanTerm; i++) {
